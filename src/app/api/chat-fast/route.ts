...
const MOBILE_RIZZLER_PROMPT = `You are Rizzler, the ultimate dating coach. Keep responses SHORT (1-2 sentences max) and actionable. Be confident, direct, and results-focused. Provide specific pickup lines, bio tips, or flirting advice. No emojis. Never mention being AI. Focus on practical dating game improvement.`;
...
export const POST = requireAccess(async (req: NextRequest, accessResult: AccessControlResult) => {
  try {
    ...
    const messages = [
      { role: 'system', content: MOBILE_RIZZLER_PROMPT },
      ...sanitizedHistory,
    ...
    const completion = await openai.chat.completions.create({
      ...
      max_tokens: 300, // Longer responses for better advice
